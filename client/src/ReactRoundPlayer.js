...
export default class ReactRoundPlayer extends Component {
  ...
  getPlaySection(position, width) {
    return (
      ...
      >
        <g>
          <polygon
            style={{ display: this.state.playing ? 'none' : true }}
            points={((size) => {
  ...
  getRectangle(x, y, width, height, color) {
    return (
      <polygon
        points={[x, y, x + width, y, x + width, y + height, x, y + height].join(
          ' '
        )}
        fill={color}
      />
  ...
  getProgressSection(position, width) {
    return (
      ...
      >
        <g>
          <text
            style={{ display: this.state.playing ? true : 'none' }}
            y={radius * 1.2}
        ...
        >
          <g>
            ...
            {this.state.progress.played === 0
              ...
              : this.getProgressArc(
                  ...
                  this.colorScale[0],
                  0,
                  this.state.playProgressAngle
                )}
          </g>
          <g
            fillOpacity="0"
            onClick={this.handleOnSeekChange}
            className={'seek-change'}
            onMouseLeave={this.handleRemoveSeekSearch}
          ...
          <g fillOpacity="0.5">
            {this.getProgressArc(
              ...
              this.colorScale[0],
              0,
              this.state.seekSearchArcAngle
            )}
          </g>
          <g
            onClick={this.handlePauseOnClick}
            className={'pause-or-play'}
            onMouseOver={this.handleShowPauseButton}
            onMouseLeave={this.handleHidePauseButton}
          >
            {this.getCircle(
              radius,
              radius,
              radius * centerWidth - border,
              this.colorScale[3]
            )}
            {this.getProgressArc(
              radius,
